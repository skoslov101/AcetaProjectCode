...
while datainputflag ~= 1
    initflag = 0;
    while initflag ~= 1
        prompt = {'Subject Number', 'Subject Initials','CondNum'};
...
cd AffectiveFlanker/
runNum=1;
[AffFlank1]=AffectiveFlankerTask(debug,subjN,init,runNum,window,affRespKeys);
cd ..
...
cd Stroop/
runNum=1;
[Stroop1]=ComputerizedStroop(debug,subjN,init,runNum,window,strRespKeys);
...
cd AffectiveFlanker/
runNum=2;
[AffFlank2]=AffectiveFlankerTask(debug,subjN,init,runNum,window,affRespKeys);
cd ..
...
cd Stroop/
runNum=2;
[Stroop2]=ComputerizedStroop(debug,subjN,init,runNum,window,strRespKeys);
cd ..
...
orderNum=1;
if rule1N==1
    [rbdata]=Aceto_150_rb(debug, subjN, init, feature1N, rule1N, orderNum,window);
else
    [iidata]=Aceto_150_ii(debug, subjN, init, feature1N, rule1N, orderNum,window);
end
...
orderNum=2;
if rule2N==1
    [rbdata]=Aceto_150_rb(debug, subjN, init, feature2N, rule2N, orderNum, window);
else
    [iidata]=Aceto_150_ii(debug, subjN, init, feature2N, rule2N, orderNum, window);
end
cd ..
% Thanks and transition
...
cd Data\
filename=['Aceto_' subjectNum '_' pinit '_' condNum '.mat'];
