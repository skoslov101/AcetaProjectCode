function varargout = GetRespTimed(varargin)
...
while ~responded
    ...
    while ~keyIsDown && (time_spanned < time_PM_term)
    ...
    elseif length(key) == 0        
        responded = 1;
        varargout = {-2,time_PM_term};
    else
        if time_spanned<time_terminate
            ...
            if isequal(upper(key), upper(varargin_keys{length(varargin_keys)}))
                responded=1;
                varargout={i-1,GetSecs-beginTime};
            end
        end
